...
public abstract class ZsonActionAbstract implements ZsonAction {
	...
	// 删除指定level及其子level的信息，信息包括level\path\index\collections中的数据
	...
	// 更新新生成的ZsonResultImpl的level\path\indexs\colletioncs信息
	protected void replaceZsonResultInfoKey(ZsonResultImpl zrNew, String targetKey, String parentPath,
			List<String> handledPath, String addRootPath) {
	...
	// 更新index中Map Value值中的index值，使之与collection的值对应
	...
	// 更新paths值,父节点路径与新路径生成最终路径
	private void updatePaths(ZsonResultImpl zrNew, Object paths, String parentPath, List<String> handledPath,
			String addRootPath) {
	...
	// 更新indexs中map的key值
	private void updateIndexs(Map<String, Map<String, Integer>> index, Map<String, Map<String, Integer>> newIndex,
			String key, String targetKey) {
		Map<String, Integer> indexInfo = index.get(key);
		// indexInfo.put(ZsonUtils.INDEX,
		// indexInfo.get(ZsonUtils.INDEX)+addIndex);
	...
	// 新的ZsonResultImpl放入老的ZsonResultImpl，生成最终ZsonResultImpl
